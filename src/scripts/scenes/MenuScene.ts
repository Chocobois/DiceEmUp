...
export class MenuScene extends BaseScene {
	...
	create(): void {
		...
		this.title = this.createText(this.CX, this.H-150, 120, "#000", "Fire and Dice");
		this.title.setOrigin(0.5, 1.0);
		this.title.setStroke("#FFF", 16);
		this.title.setVisible(false);
		this.title.setAlpha(0);

		this.subtitle = this.createText(this.CX, this.H-100, 70, "#000", "Tap to start");
		this.subtitle.setOrigin(0.5);
		this.subtitle.setStroke("#FFF", 8);
		this.subtitle.setVisible(false);
		this.subtitle.setAlpha(0);

		this.tap = this.createText(this.CX, this.CY, 70, "#000", "Tap to focus");
		...
		let credits1 = this.createText(14, 8, 28, "#c2185b", creditsLeft);
		credits1.setStroke("#FFF", 6);
		credits1.setLineSpacing(0);
		this.credits.add(credits1);

		let credits2 = this.createText(240, 8, 28, "#c2185b", creditsRight);
		credits2.setStroke("#FFF", 6);
		credits2.setLineSpacing(0);
		this.credits.add(credits2);

		this.version = this.createText(this.W - 8, this.H, 24, '#c2185b', version);
		this.version.setOrigin(1);
		this.version.setAlpha(0);
		this.version.setStroke('#FFF', 6);
		this.version.setLineSpacing(-36);
		this.version.setPadding(4);
	...
	update(time: number, delta: number) {
		if (this.dragon.visible) {
			this.dragon.y		+= 0.04 * (this.CY - this.dragon.y);
			this.roof.y			+= 0.05 * (this.CY - this.roof.y);
			this.hands.y		+= 0.05 * (this.CY - this.hands.y);

			this.roof.x			+= 0.02 * ((this.CX + 12 * Math.cos(1*0.4*time/1000)) - this.roof.x);
			this.hands.x		+= 0.02 * ((this.CX + 14 * Math.cos(1*0.4*time/1000)) - this.hands.x);

			this.diceBlue.x		+= 0.06 * ((this.CX + 10 * Math.sin(time/1000+Math.PI/4)) - this.diceBlue.x);
			this.diceGreen.x	+= 0.06 * ((this.CX - 10 * Math.sin(time/1000)) - this.diceGreen.x);
			this.diceRed.x		+= 0.06 * ((this.CX - 14 * Math.sin(time/1000+Math.PI/3)) - this.diceGreen.x);
			this.diceBlue.y		+= 0.06 * ((this.CY + 20 * Math.sin(0.6*time/1000+Math.PI/4)) - this.diceBlue.y);
			this.diceGreen.y	+= 0.06 * ((this.CY - 20 * Math.sin(0.6*time/1000)) - this.diceGreen.y);
			this.diceRed.y		+= 0.06 * ((this.CY - 14 * Math.sin(0.6*time/1000+Math.PI/3)) - this.diceGreen.y);

			this.knights.x		+= 0.02 * ((this.CX + 30 * Math.cos(4*0.4*time/1000)) - this.knights.x);
			...
			this.tap.alpha += 0.02 * (1 - this.tap.alpha);
